...
Gate 1 is authority only, Gate 2 adds one command-local help authority, and
Gate 3 adds private actionable usage presentation plus one-shot Edit stdin
Content. Its
...
only to the Adapter error presentation; `--stdin` is an exclusive EOF Content
source after argv/v5 validation and before Runtime access, not a Core input,
wire, state, or raw-Session transport. Future references stay
